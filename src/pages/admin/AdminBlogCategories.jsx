...
const AdminBlogCategories = () => {
  ...
  /* ─────────────────────────────────────────
     DELETE
  ───────────────────────────────────────── */
  const openDelete = (id, name) => { setDeleteId(id); setDeleteName(name); setDeleteOpen(true); };

  const confirmDelete = async () => {
    if (!deleteId) return;
    setDeleteBusy(true);
    try {
      const res = await fetch(`${API_BASE_URL}/admin/blog-categories/${deleteId}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
      if (!res.ok) throw new Error('Failed');
  ...
  useEffect(() => {
    const h = (e) => {
      if (e.key === 'Escape') { setEditOpen(false); setDeleteOpen(false); }
    };
    ...
    <div className="min-h-screen bg-[#f2f5f9] font-['DM_Sans']">
      ...
      <div>
        ...
        {/* ══════════════════════════════════════════
            ...
            .readonly-notice: bg #fffbeb, border 1px solid #fde68a,
            border-radius 14px, padding 12px 16px, mb 18px,
            font-size 13px, color #92400e, flex items-center gap 8px
        ══════════════════════════════════════════ */}
        {isSubAdmin && (
          <div
          ...
          >
            ...
            <span>
              You have <strong>view-only</strong> access to blog categories. Contact an Admin to add, edit or delete categories.
            </span>
          </div>
        )}
        ...
        {/* ══════════════════════════════════════════
            ...
            .add-card: white, radius 14px, border #e8ecf1,
            padding 20px 22px, mb 20px, shadow
        ══════════════════════════════════════════ */}
        {!isSubAdmin && (
          <div
          ...
          >
            ...
            {/* Row: input (col-md-10) + button (col-md-2) */}
            <div className="flex flex-col sm:flex-row gap-3 items-end">
              ...
              {/* .btn-teal-add: full width on its col, 9px 20px pad, teal, radius 99px, 13.5px, 600 */}
        ...
        {/* ══════════════════════════════════════════
            TABLE CARD
            .table-card: white, radius 14px, border, overflow-hidden, shadow
        ══════════════════════════════════════════ */}
        <div
          ref={tableCardRef}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              {/* .search-box: flex items-center gap-2, bg #f8fafc, border 1.5px #e8ecf1,
                   radius 99px, padding 6px 14px */}
              <div
              ...
              <thead style={{ background: '#f8fafc' }}>
                <tr>
                  {[
                    ...
                    { label: 'Created',        w: 150 },
                    { label: 'Actions',        w: isSubAdmin ? 120 : 160 },
                  ].map(({ label, w }) => (
                    <th
                      key={label}
                      style={{
              ...
              <tbody className="bc-tbody">
                ...
                {!loading && !error && pageData.map((cat, i) => (
                  <tr key={cat.id ?? cat.categoryId ?? i}>
                    ...
                    {/* Category Name — avatar + .cat-badge */}
                    <td style={{ padding: '12px 16px', borderBottom: '1px solid #f8fafc' }}>
                      <div className="flex items-center gap-2.5">
                        ...
                        {/* .cat-badge: inline-flex gap-1.5, text-[13px], font-medium,
                             py-[5px] px-3, radius-[20px], bg #eff6ff, color #1d4ed8 */}
                        <span
                    ...
                    <td style={{ padding: '12px 16px', borderBottom: '1px solid #f8fafc' }}>
                      {isSubAdmin ? (
                        ...
                        <div className="flex gap-1.5 flex-wrap">
                          {/* .action-btn.btn-edit: color #d97706, border-color #fde68a, bg #fffbeb */}
                          <button
                          ...
                          {/* .action-btn.btn-delete: color #dc2626, border-color #fecaca, bg #fef2f2 */}
                          <button
                            onClick={() => openDelete(cat.id ?? cat.categoryId, cat.name)}
                            className="inline-flex items-center gap-1 text-[12px] font-semibold cursor-pointer rounded-lg transition-all font-['DM_Sans'] border-[1.5px]"
                            style={{ padding: '5px 12px', color: '#dc2626', borderColor: '#fecaca', background: '#fef2f2' }}
          ...
          {/* PAGINATION WRAPPER
              .pagination-wrapper: padding 14px 20px, border-top #f1f5f9,
              flex justify-between items-center flex-wrap gap-2.5 */}
          <div
            className="flex justify-between items-center flex-wrap gap-2.5"
      ...
      {/* ══════════════════════════════════════════
          DELETE MODAL
          Red header (#b91c1c), modal-sm (max-w-sm), centered body
      ══════════════════════════════════════════ */}
      {deleteOpen && (
        <div
          className="fixed inset-0 z-[500] flex items-center justify-center p-4"
        ...
        >
          <div
            className="bg-white w-full max-w-[380px] rounded-[20px] overflow-hidden"
            style={{
              boxShadow: '0 24px 64px rgba(11,34,57,0.18)',
              animation: 'modalIn .3s cubic-bezier(.34,1.56,.64,1)',
            }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Red header — exact: bg #b91c1c, padding 20px 24px, flex between items-center */}
            <div
              className="flex items-center justify-between"
              style={{ background: '#b91c1c', padding: '20px 24px' }}
            >
              <div className="flex items-center gap-3">
                {/* icon box: 40px, radius 10px, rgba(255,255,255,0.15), color white, 18px */}
                <div
                ...
                <div>
                  <p className="text-[15px] font-extrabold text-white m-0">Delete Blog Category</p>
                  <p className="text-[12px] text-white/60 m-0">This action cannot be undone</p>
                </div>
              </div>
              {/* close btn — same pm-close-btn style */}
              <button
                onClick={() => setDeleteOpen(false)}
                className="bc-close-btn flex items-center justify-center cursor-pointer transition-all"
                style={{
                  width: 32, height: 32, borderRadius: 8,
                  background: 'rgba(255,255,255,0.08)',
                  border: '1px solid rgba(255,255,255,0.12)',
            ...
            {/* .pm-body text-center, padding 28px 24px */}
            <div className="text-center" style={{ padding: '28px 24px' }}>
              {/* warning icon */}
              <AlertTriangle
                size={40}
                className="text-red-500 block mx-auto mb-[14px]"
              />
              <p className="text-[14px] text-[#0f172a] mb-1">Are you sure you want to delete</p>
              <p className="text-[15px] font-bold text-[#dc2626] mb-2">{deleteName}</p>
              <p className="text-[12.5px] text-[#64748b]">
                This may affect existing blogs using this category.
              </p>

              {/* .pm-footer justify-center */}
              <div
                className="flex justify-center gap-2.5 mt-5 pt-4"
                style={{ borderTop: '1px solid #e8ecf1' }}
              >
                <button
                  onClick={() => setDeleteOpen(false)}
                ...
                {/* .pm-btn.primary with bg #dc2626 override */}
                <button
                  onClick={confirmDelete}
                  disabled={deleteBusy}
                  className="inline-flex items-center gap-[7px] px-[22px] py-[10px] rounded-full text-[13.5px] font-semibold text-white cursor-pointer border-none transition-all font-['DM_Sans'] disabled:opacity-60 disabled:cursor-not-allowed"
                  style={{ background: '#dc2626' }}
                  onMouseEnter={(e) => { if (!deleteBusy) { e.currentTarget.style.background = '#b91c1c'; e.currentTarget.style.boxShadow = '0 6px 18px rgba(220,38,38,0.28)'; } }}
                  onMouseLeave={(e) => { e.currentTarget.style.background = '#dc2626'; e.currentTarget.style.boxShadow = ''; }}
                >
                  {deleteBusy && <Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} />}
