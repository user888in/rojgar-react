...
const RecruiterRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/auth/recruiter/register`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fullName: form.fullName, email: form.email, companyName: form.companyName, password: form.password }),
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f1f5f9] px-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <h1 className="text-2xl font-bold text-[#091d33] text-center mb-2">Recruiter Registration</h1>
        <p className="text-sm text-[#64748b] text-center mb-6">Create your recruiter account</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="email" name="email" value={form.email} onChange={handleChange} required
              className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-[#18a99c]" placeholder="you@company.com" />
          </div>
          <div>
            <label className="block text-sm font-medium text-[#091d33] mb-1">Company Name</label>
            <input type="text" name="companyName" value={form.companyName} onChange={handleChange} required
              className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-[#18a99c]" placeholder="Your company" />
          </div>
          <div>
        ...
        <p className="mt-6 text-center text-sm text-[#64748b]">
          Already have an account? <Link to="/recruiter/login" className="text-[#18a99c] hover:underline font-medium">Sign in</Link>
        </p>
      </div>
    </div>
  );
